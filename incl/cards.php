<!-- NOTE Legende -->
<div class="legende modal fade" id="legende" tabindex="-1" role="dialog" aria-hidden="true">
   <div class="modal-dialog">
      <div class="modal-content">
         <div class="close-modal" data-dismiss="modal">
            <div class="lr">
               <div class="rl"></div>
            </div>
         </div>
         <div class="container">
            <div class="row">
               <div class="col-lg-8 mx-auto">
                  <div class="modal-body">
                     <!-- Details Go Here -->
                     <h2 class="text-uppercase">Legende zu Bildfarben und Formen</h2>
                     <p class="item-intro text-muted">
                        Lichtschranke: Kreis -> orange: #f78a15 -> #b96306<br>
                        Motor: … -> lila: #7800bd -> #470071<br>
                        Taster/Anschlag: … -> pink: #fd76ff -> #fc2aff<br>
                        Schalter: … -> grün: #427c42<br>
                        Leuchte: … -> gelb: #ffd921<br>

                     </p>


                     <button class="btn btn-outline-secondary mt-5" data-dismiss="modal" type="button">
                        <i class="fas fa-times"></i>
                        Schließen
                     </button>
                  </div>
               </div>
            </div>
         </div>
      </div>
   </div>
</div>